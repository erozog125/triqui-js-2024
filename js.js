...
function playMachine() {
    ...
    validateGame();
    isUserTurn = true; // Ahora es el turno del usuario
}

function handleUserClick(idx) {
    if (!isGameOver && isUserTurn && board[idx].textContent === '') {
        board[idx].textContent = user;
        boardGame[idx] = user;
        validateGame();
        if (!isGameOver) {
            isUserTurn = false; // Turno de la máquina
            setTimeout(() => {
                playMachine();
            }, 1000);
        }
...
function resetGame() {
    ...
    modal.style.display = 'flex'; // Mostrar el modal de selección nuevamente
}
